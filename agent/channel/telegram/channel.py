...
class TelegramChannel(BaseChannel):
    ...
    def send_broadcast(self, message: str) -> None:
        """Send agent-initiated broadcast to all known Telegram chats. Blocks until sent to preserve order."""
        if not self._loop or not self._application:
            return
        chat_ids = self._target_chat_ids()
        if not chat_ids:
        ...
        try:
            ...
            log(f"[Telegram send_broadcast wait] {e}")
    ...
    def run(self, agent) -> None:
        ...
        async def handle_command(update: Update, context: ContextTypes.DEFAULT_TYPE, cmd_name: str) -> None:
            chat_id = update.effective_chat.id if update.effective_chat else None
            if chat_id is not None:
                self._chat_ids.add(chat_id)
        ...
        def make_cmd_handler(name: str):
            async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
                await handle_command(update, context, name)
            return handler

        for cmd_name, _ in COMMANDS:
            app.add_handler(CommandHandler(cmd_name, make_cmd_handler(cmd_name)))
